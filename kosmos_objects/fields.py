...
#TODO (graham): Remove this when https://review.openstack.org/#/c/250493 merges
...
class UUIDField(fields.AutoTypedField):
    def __init__(self, validate=None, **kwargs):
        self.AUTO_TYPE = UUID(validate=validate, **kwargs)
...
class PreDefinedEnumType(EnumField):
    ...
    _TYPES = ()
    _msg = _("%s is not a valid choice, choose from %r")

    def __init__(self, **kwargs):
        super(PreDefinedEnumType, self).__init__(PreDefinedEnumType._TYPES,
                                                 **kwargs)

    def coerce(self, obj, attr, value):
        try:
            return super(PoolMemberType, self).coerce(obj, attr, value)
        except ValueError:
            msg = self._msg % (value, self._TYPES)
            raise ValueError(msg)


class PoolMemberType(EnumField):
...
class MonitorType(EnumField):
    ...
    def __init__(self, **kwargs):
        super(MonitorType, self).__init__(MonitorType._TYPES, **kwargs)

    def coerce(self, obj, attr, value):
        try:
            return super(MonitorType, self).coerce(obj, attr, value)
        except ValueError:
            msg = _("%s is not a valid Monitor Type, choose from %r") % \
                  (value, self._TYPES)
            raise ValueError(msg)
